...
namespace vapaee {
    ...
    class [[eosio::contract("telosbookdex")]] telosbookdex : public eosio::contract {
        ...
        public:
            ...
            // Maintenance Module
            [[eosio::action]]
            void maintenance (name credits_to) {
                MAINTENANCE();
            ...
            AUX_DEBUG_CODE (
                ...
                [[eosio::action]]
                void testdao (name ballot_name, map<name, asset> final_results, uint32_t total_voters) {
                    MAINTENANCE();
                    PRINT("\nACTION telosbookdex.testdao() ------------------\n");
                    handle_dex_broadcast(ballot_name, final_results, total_voters);
                };

                [[eosio::action]]
                void hotfix (int max, name scope, asset q) {
                    PRINT("\nACTION telosbookdex.hotfix() ------------------\n");
                    vapaee::dex::exchange::action_hotfix(max, scope, q);
                };

                [[eosio::action]]
                void timeoffset (uint32_t offset) {
